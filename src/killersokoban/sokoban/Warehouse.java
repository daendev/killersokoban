...
public class Warehouse {
    ...
    public void save(String fileName){
        try {
            ...
            file.write(new StringBuilder().append(this.getMapWidth()).append(" ").append(this.getMapHeight()).append("\n").toString());
            this.drawForSave(file);
            file.write(new StringBuilder().append(this.getPlayers().size()).append("\n").toString());
            for (int i=0; i<this.getPlayers().size(); i++) {
                file.write(new StringBuilder().append(this.getPlayers().get(i).getPlace().getX()).append(" ").toString());
                file.write(new StringBuilder().append(this.getPlayers().get(i).getPlace().getY()).append("\n").toString());
            }

            file.write(new StringBuilder().append(this.getBoxes().size()).append("\n").toString());
            for (int i=0; i<this.getBoxes().size(); i++) {
                file.write(new StringBuilder().append(this.getBoxes().get(i).getPlace().getX()).append(" ").toString());
                file.write(new StringBuilder().append(this.getBoxes().get(i).getPlace().getY()).append("\n").toString());
            }

            for (int i = 0; i < this.getSwitches().size(); i++) {
                file.write( new StringBuilder().append(this.getSwitches().get(i).getHole().getX()).append(" ").toString());
                file.write( new StringBuilder().append(this.getSwitches().get(i).getHole().getY()).append("\n").toString());
            }
