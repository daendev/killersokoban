...
import killersokoban.graphics.GraphicsCollection;
...
import killersokoban.sokoban.Directions;
import killersokoban.sokoban.Player;
import killersokoban.sokoban.Warehouse;
...
public class GameController {
    ...
    private void initScoreboard(int players){
        ...
        for(int i = 1; i <= players; ++i){
            Label playerString = new Label(new StringBuilder().append("Player ").append(i).toString());
            playerString.setId(new StringBuilder().append("player").append(i).append("string").toString());
            scorePane.add(playerString, 0, i);
            Label playerScore = new Label("0");
            playerScore.setId(new StringBuilder().append("player").append(i).append("score").toString());
    ...
    private void updateScore(){
        ...
        for(Player p : warehouse.getPlayers()){
            String lookupString = new StringBuilder().append("#player").append(i).append("score").toString();
            ((Label)scorePane.lookup(lookupString)).setText(String.valueOf(p.getScore()));
            ++i;
        }
    }
